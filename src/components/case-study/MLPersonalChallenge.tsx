...
export default function MLPersonalChallenge({ isLightBackground = true }: MLPersonalChallengeProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-[var(--accent-teal-50)] border-l-4 border-[var(--accent-teal)] p-6 md:p-8 rounded-r-lg"
    >
      <div className="space-y-6">
        {/* Header */}
        <div className="flex items-start gap-4">
          <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-[var(--accent-teal-100)] flex items-center justify-center">
            <BrainCircuit className="w-6 h-6 text-[var(--accent-teal)]" />
          </div>
          <div className="space-y-2">
            <h4 className="font-mono text-sm uppercase tracking-widest text-[var(--accent-teal-700)]">
              CHALLENGE: THE_KNOWLEDGE_GAP
            </h4>
            <p className="text-slate-800 text-lg md:text-xl leading-relaxed font-serif">
              I entered this project with <span className="font-semibold text-[var(--accent-teal-700)]">zero ML knowledge</span>. 
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
          {learningMethods.map((method, index) => (
            <motion.div
              ...
              className="bg-white/80 rounded-lg p-4 border border-[var(--accent-teal-100)]"
            >
              <method.icon className="w-5 h-5 text-[var(--accent-teal)] mb-2" />
        ...
        {/* Outcome Note */}
        <div className="bg-white/50 rounded-lg p-4 border border-[var(--accent-teal-100)] mt-4">
          <p className="text-slate-700 text-sm leading-relaxed">
            <span className="font-mono text-[var(--accent-teal)] text-xs">&gt; OUTCOME:</span> Within weeks, I became the 
